...
class BackpropKF_Solver(General_Solver):
    ...
    def train_step(self, batch_sample):
        ...
        for i, seq in enumerate(batch_sample):
            ...
            rpn_proposals, instances, tracks, rpn_losses, detection_losses, track_loss = self.model(
                in_images, target, self.is_training)
        ...
        with torch.no_grad():
            for idx, batch_sample in enumerate(self.val_loader):
                for seq in batch_sample:
                    ...
                    evaluator.evaluate(in_images, instances, targets)
                    evaluator_KF.evaluate(in_images, tracks, targets)
            ...
            print("KF")
            evaluator_KF.plot(self.exp_dir)
            evaluator_KF.print()