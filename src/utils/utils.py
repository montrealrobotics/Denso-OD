...
def get_actual_coords(prediction, anchors):
	...
	uncertainty = prediction[2].detach().cpu().numpy()
	...
	prob = pred[0,:,1]
	uncertainty = np.linalg.norm(uncertainty[0,:,:], axis=1)
	# print(prob.shape)

	x1 = x_c - w/2.0
	y1 = y_c - h/2.0

	bbox_locs_xy = np.vstack((x1, y1, x1+w, y1+h, prob, uncertainty)).transpose() ## Final locations of the anchors
