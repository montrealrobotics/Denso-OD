...
from . import utils
# from third_party.pykitti import utils - this works but it's not relative
# import utils - This one doesn't work
...
class tracking:
    ...
    def load_calib(self):
        ...
        # Create 3x4 projection matrices
        P_00 = np.reshape(filedata['P0'], (3, 4))
        P_10 = np.reshape(filedata['P1'], (3, 4))
        P_20 = np.reshape(filedata['P2'], (3, 4))
        P_30 = np.reshape(filedata['P3'], (3, 4))


        # This rectification is just rotation matrix for other cameras wrt cam0
        R_rect_00 = np.reshape(filedata['R_rect'], (3,3))
        R_rect_00 = np.vstack((R_rect_00, np.array([0,0,0])))
        R_rect_00 = np.column_stack((R_rect_00, np.array([0,0,0,1])))
        
        Tr_velo_to_cam0 = np.reshape(filedata['Tr_velo_cam'], (3,4))
        Tr_velo_to_cam0 = np.vstack((Tr_velo_to_cam0, np.array([0,0,0,1])))

        data['Tr_velo_to_cam0'] = Tr_velo_to_cam0
